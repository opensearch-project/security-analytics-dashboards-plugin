...
const GlobalFilterForm = (props) => {
  ...
  const onFieldSearchChange = (searchValue) => {
    setFieldOptions(
      fieldOption.filter((option) => option.label.toLowerCase().includes(searchValue.toLowerCase()))
    );
  };
  ...
  const onValuesSearchChange = (searchValue) => {
    setValueOptions(
      valueOption.filter((option) => option.label.toLowerCase().includes(searchValue.toLowerCase()))
    );
  };
  ...
  return (
    <div {...rest}>
      <EuiFlexGroup>
        <EuiFlexItem style={{ maxWidth: '188px' }}>
          <EuiFormRow label="Field">
            <EuiComboBox
              placeholder={selectedOperand.length < 1 ? 'Start here' : 'Select a field'}
        ...
        <EuiFlexItem style={{ maxWidth: '188px' }}>
          <EuiFormRow label="Operand">
            <EuiComboBox
              placeholder={selectedField.length < 1 ? 'Select a field first' : 'Select an operand'}
      ...
      <EuiFlexGroup direction="rowReverse" alignItems="center">
        ...
        <EuiFlexItem grow={false}>
          <EuiButtonEmpty flush="right" onClick={selectedObject ? onCancel : resetForm}>
