...
export interface DetectorDetailsProps extends RouteComponentProps<
  {},
  any,
  {
...
> {
  ...
  getDetector = async () => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const dashboard =
          await this.props.savedObjectsService.getDashboard(detectorId);
        if (dashboard?.id) {
  ...
  onDelete = async () => {
    ...
    try {
      ...
      const index =
        await this.props.indexPatternsService.getIndexPattern(detectorId);
      if (index) {
